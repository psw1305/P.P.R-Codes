...
namespace PSW.Core.Map
{
    public static class MapGenerator
    {
        ...
        public static Map GetMap(MapConfig conf)
        {
            ...
            var bossNodeName = config.nodeBlueprints.Where(b => b.GetNodeType() == MapNodeType.Boss).ToList().Random().name;
        ...
        private static void PlaceLayer(int layerIndex)
        {
            ...
            var offset = layer.nodesApartDistance * config.GridWidth / 3;

            for (var i = 0; i < config.GridWidth; i++)
            {
                var nodeType = Random.Range(0f, 1f) < layer.randomizeNodes ? GetRandomNode() : layer.nodeType;
                var blueprintName = config.nodeBlueprints.Where(b => b.GetNodeType() == nodeType).ToList().Random().name;
        ...
        private static void RemoveCrossConnections()
        {
            for (var i = 0; i < config.GridWidth - 1; ++i)
                for (var k = 0; k < config.layers.Length - 1; ++k)
                {
                    var node = GetNode(new Point(i, k));
                    if (node == null || node.HasNoConnections()) continue;
                    var right = GetNode(new Point(i + 1, k));
                    if (right == null || right.HasNoConnections()) continue;
                    var top = GetNode(new Point(i, k + 1));
                    if (top == null || top.HasNoConnections()) continue;
                    var topRight = GetNode(new Point(i + 1, k + 1));
        ...
        private static MapNodeType GetRandomNode()
        {
